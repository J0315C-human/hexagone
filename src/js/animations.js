...
const cols = {
	hex_A: '#aa6065',
	hex_A_flash: Color('#aa6065').lighten(0.6).rgb().string(),
	hex_B:'#5392a5',
	hex_B_flash: Color('#5392a5').lighten(0.6).rgb().string(),
};

const _s = glob.animationScale;
const _flashRuleA = CSSRulePlugin.getRule('.hex-flash');
const _flashRuleB = CSSRulePlugin.getRule('.hex-flash.hex-buffer');

const _flashLength = glob.beat_ms - glob.preschedule_ms;
const _flashIn = (_flashLength * 0.1) / 1000;
const _flashOut = (_flashLength * 0.9) / 1000;
const _timeline = new TimelineLite({ autoRemoveChildren: true });
const _altTimeline = new TimelineLite({ autoRemoveChildren: true }).play();
const _hexDelay = glob.preschedule_ms / 1000;
const _otherDelay = glob.preschedule_ms / 1000;
